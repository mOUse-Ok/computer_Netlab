 ...
 * 缓冲区大小
 * 含义：通用数据缓冲区的字节大小
 * 修改方法：通常不需要修改，保持与MAX_PACKET_SIZE一致
 * 修改效果：
 *   - 过小：可能导致数据截断
 *   - 过大：浪费内存
 */
 ...
 *   - 丢包率高的网络：适当减小到2-4
 */
#define FIXED_WINDOW_SIZE 4

/**
 * 最大报文段大小（MSS）
 * 含义：数据缓冲区的单位大小
 * 修改方法：保持与MAX_PACKET_SIZE一致
 * 修改效果：
 *   - 影响窗口内数据缓冲区的分配
 */
#define MSS 1024
...
/**
 * 心跳间隔（毫秒）
 * 含义：保活机制的心跳发送间隔（当前版本未使用）
 * 修改方法：建议范围500-5000ms
 * 修改效果：
 *   - 较短间隔：更快检测连接断开，但增加网络流量
 *   - 较长间隔：减少网络开销，但检测断开较慢
 */
 ...
 *   - false：关闭丢包模拟，正常传输
 * 修改效果：
 *   - 启用时：会丢弃g_lossSeq指定的包，触发重传
 *   - 关闭时：所有包正常处理
 * 
 * 注意：此参数在server.cpp中作为全局变量定义
 *      g_simulateLoss = true/false
 *      g_lossSeq = clientSeq + N （N为要丢弃的第N个包）
 */
// 丢包模拟参数说明（在server.cpp中配置）：
// bool g_simulateLoss = true;     // 是否启用丢包模拟
// uint32_t g_lossSeq;             // 要丢弃的序列号（如 clientSeq + 2 表示第2个包）
